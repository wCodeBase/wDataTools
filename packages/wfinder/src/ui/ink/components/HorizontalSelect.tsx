...
export const HorizontalSelect = <T extends unknown>(props: {
  ...
  const [state, setState] = useStableState(() => ({
    selected:
      props.options.find((v) => v.value === props.defaultSelected) ||
      props.autoSelectFirstOption
        ? props.options[0]
        : undefined,
  }));
  ...
  useInput((val, key) => {
    if (key.tab || key.rightArrow || key.leftArrow || key.return) {
      let optionIndex = state.selected
        ? props.options.findIndex((v) => v.value === state.selected?.value)
        : -1;
      if (key.return) {
  ...
  return (
    <Box display="flex" flexDirection={props.oneLine ? "row" : "column"}>
      {!!props.label && (
        <Text color={props.danger ? "yellow" : "blue"}>{props.label}</Text>
      )}
      <Box marginLeft={2}>
        {props.options.map((op) => (
          <Box key={op.value + ""} marginRight={2}>
            <Text
              backgroundColor={op.value === state.selected?.value ? "blue" : ""}
            >
              {op.label}
            </Text>
          </Box>
