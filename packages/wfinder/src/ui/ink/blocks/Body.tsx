...
const Scan = () => {
    ...
    if (!state.confirmed) return <Confirm horizontal message="Start scanning?" onSelect={(confim) => {
        if (confim) {
...
const PathManage = () => {
    ...
    const cmdOptions = useMemo(()=>[...((state.paths.find(v=>v.id===state.selectedId)) ? ["delete highlight path"] : []), "add new path"].map(v => ({ label: v, value: v })),[state.paths,state.selectedId])
    ...
    useSubjectCallback(EvUiCmdResult, msg => {
        if (msg.cmd === 'deletePath' || msg.cmd === 'addPath') {
            ...
            setState({ paths: msg.result.results, skip: Math.max(0,Math.min(state.skip, msg.result.results.length - state.showLength)) });
    ...
    return <>
        ...
        {!state.cmd ? <>
            ...
            : state.cmd.includes("delete") ? <Confirm danger horizontal
            ...
            />
                : state.cmd.includes('new') && <>
                    {!state.newPath ? <CmdInput initialValue={state.newPath} label="New path" placeholder="Input new path here" onSubmit={newPath => setState({ newPath })} />
                        : <Confirm horizontal message={`Add this path?: ${state.newPath}`} onSelect={confirm => {
                            if (confirm) {
