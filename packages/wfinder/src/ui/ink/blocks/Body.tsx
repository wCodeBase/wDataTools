...
const Search = () => {
    ...
    const [state, setState] = useStableState(() => ({
        ...
        doSearch: () => {
            if (state.keywords.every(v => v)) EvUiCmd.next({ cmd: 'search', data: { ...state } });
        }
    }));
    ...
    useCliKeyPress(key => {
        if (state.keywords && state.records?.length) {
            if (subject.value === FinderState.searching) EvLog("Busy searching now, try page up or down later.")
                ...
                if (state.skip + state.take < state.total) {
                    setState({ skip: Math.min(state.skip + state.take, state.total - state.take) });
                    state.doSearch();
...
const Scan = () => {
    const [finderState, subject] = useFinderState();
    const [state, setState] = useStableState(() => ({
        confirmed: finderState === FinderState.scanning, goingBack: false,
        ...
        onSelect: (option: { value: string }) => {
            ...
            if (option.value.includes("stop")) EvUiCmd.next({ cmd: 'stopScan', data: null });
            if (option.value.includes("scan again")) EvUiCmd.next({ cmd: 'scan', data: null });
        }
    }));
    const items = useMemo(() => {
        const options = finderState === FinderState.scanning ? ['stop', 'stop and goback'] : ['goback', 'scan again'];
    ...
    useEffect(() => {
        if (state.goingBack && finderState === FinderState.idle) triggerCliKeyCallback(KEY_ESCAPE);
    }, [finderState, state.goingBack]);

    if (!state.confirmed) return <Confirm message="Start scanning?" onSelect={(confim) => {
        if (confim) {
    ...
    return <>
        {<Box marginBottom={1} display="flex" flexDirection="column">
            ...
            {!state.scanMessages.length ? <Text>No message output yet.</Text>
                : state.scanMessages.slice(-state.scanMsgShowLines).map(msg =>
                    <Text key={msg.number} color={msg.error ? 'yellow' : "white"}>{`(${msg.number}) ${msg.message} ${msg.error || ''}`}</Text>)}
        </Box>}
        <Text>Usable options {finderState === FinderState.scanning ? '(scaning)' : state.confirmed ? "(scan done)" : ''}:</Text>
...
const PathManage = () => {
    ...
    useSubjectCallback(EvUiCmdResult, msg => {
        if (msg.cmd === 'deletePath' || msg.cmd === 'addPath') {
            ...
            setState({ paths: msg.result.results, skip: Math.min(state.skip, msg.result.results.length - state.showLength) });
            if (msg.result.error) EvLog("Error: " + msg.result.error);
            if (state.waitingForCmdResult) setState({ waitingForCmdResult: false, cmd: '' });
        }
    });
    ...
    useCliKeyPress(key => {
        if (key === KEY_ARROW_UP || key === KEY_ARROW_DOWN || key === KEY_PAGE_UP || key === KEY_PAGE_DOWN) {
            if (state.cmd.includes("")) {
                let currentIndex = -1;
                let skip = state.skip;
                const viewEnd = Math.min(skip + state.showLength, state.paths.length) - 1;
                if (key === KEY_ARROW_UP || key === KEY_ARROW_DOWN) {
                    currentIndex = state.paths.findIndex(v => v.id === state.selectedId);
                    if (currentIndex < skip || currentIndex > viewEnd) {
                        currentIndex = key === KEY_ARROW_UP ? skip : viewEnd;
                    } else {
                    ...
                    if (key === KEY_PAGE_UP) {
                        ...
                        skip = Math.min(state.paths.length - state.showLength, skip + state.showLength);
                    }
                }
    ...
    // TODO: use horizontal select to aviod key event conflict
    return <>
        <Box display="flex" flexDirection="column" marginBottom={1}>
            <Text color="blue">Scan path list:</Text>
            {!state.paths.length ? <Text>No scan path exist yet.</Text>
                : state.paths.slice(state.skip, state.skip + state.showLength).map((v, i) =>
                    <Text color={v.id === state.selectedId ? 'blue' : 'white'}>{`(${state.skip + i}) ${v.path}`}</Text>)
            }
        </Box>

        {!state.cmd ? <>
            <Text>Usable options:</Text>
            <SelectInput items={[...(state.paths.length ? ["delete highlight path"] : []), "add new path"].map(v => ({ label: v, value: v }))} onSelect={state.onSelect} />
        </>
            : state.cmd.includes("delete") ? <Confirm danger
                message={`Sure to delete path: ${state.paths.find(v => v.id === state.selectedId)?.path}`}
                onSelect={confirm => {
                    if (state.checkBusy()) return;
                    if (confirm) {
                        EvUiCmd.next({ cmd: 'deletePath', data: [state.paths.find(v => v.id === state.selectedId)?.path || ''] });
                        setState({ waitingForCmdResult: true });
                    } else setState({ cmd: '' });
                }}
            />
                : state.cmd.includes('new') && <>
                    {!state.newPath ? <CmdInput initialValue={state.newPath} label="New path" placeholder="Input new path here" onSubmit={newPath => setState({ newPath })} />
                        : <Confirm message={`Add this path?: ${state.newPath}`} onSelect={confirm => {
                            if (confirm) {
...
const Commands = (props: { onSelect: (cmd: TypeUiCmd) => void }) => {
    return <>
        ...
        <SelectInput
            ...
            items={Object.entries(UI_CMD_DEF).map(([value, label]) => ({ value, label }))} />
    </>
}
...
export const Body = () => {
    ...
    return <Text>{`ERROR: render for command "${state.cmd}" is not found.`}</Text>;
}
