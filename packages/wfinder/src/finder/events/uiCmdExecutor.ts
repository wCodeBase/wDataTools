import { exAddScanPath, exDeleteScanPath, exListScanPath } from './../executors';

import { FileInfo } from '../entities/FileInfo';
import { doScanCmd, stopScan } from './../scan';
...
EvUiCmd.subscribe(async msg=>{
    if(msg.cmd === 'scan'){
        ...
        const records = (await FileInfo.findByMatchName(data.keywords, data.take, data.skip)).map(v=>{
        ...
        EvUiCmdResult.next({cmd, result:{results: result.map(v=>v.toItem()), error:''}});
    }
})